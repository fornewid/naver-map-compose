...
import land.sungbin.navermap.compose.options.CameraUpdateReason.Companion.fromInt
import land.sungbin.navermap.compose.options.CameraUpdateReason.NO_MOVEMENT_YET
import land.sungbin.navermap.compose.options.CameraUpdateReason.UNKNOWN
...
public enum class CameraUpdateReason(public val value: Int) {
  UNKNOWN(2),
  NO_MOVEMENT_YET(1),
  DEVELOPER(CameraUpdate.REASON_DEVELOPER),
  GESTURE(CameraUpdate.REASON_GESTURE),
  CONTROL(CameraUpdate.REASON_CONTROL),
  LOCATION(CameraUpdate.REASON_LOCATION),
  ;

  public companion object {
    public fun fromInt(reason: Int): CameraUpdateReason =
      entries.firstOrNull { it.value == reason } ?: UNKNOWN
  }
}
