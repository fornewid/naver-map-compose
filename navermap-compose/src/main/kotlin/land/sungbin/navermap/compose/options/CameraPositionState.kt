 ...
 * [CameraPositionState]는 단일 [MapView]의 상태를 반영하므로 한 번에 하나의 [land.sungbin.navermap.compose.NaverMap]
 * composable에서만 사용할 수 있습니다.
 *
 * @param position 초기 좌표를 지정합니다.
 */
public class CameraPositionState(position: CameraPosition = NaverMap.DEFAULT_CAMERA_POSITION) {
  /**
   * 카메라가 현재 움직이고 있는지 여부입니다. 여기에는 이동, 확대/축소 또는 회전과 같은 모든
   * 종류의 이동이 포함됩니다.
   */
  ...
  /**
   * 이 지도의 화면 좌표와 위도/경도 간의 변환에 사용할 [Projection] 객체를 반환합니다.
   * 항상 같은 객체가 반환됩니다.
   */
  ...
  /**
   * 콘텐츠 패딩을 포함한 지도의 뷰 전체 영역에 대한 좌표열을 반환합니다. 좌표열은 네 개의
   * 좌표로 구성된 사각형으로 표현됩니다. 단, 반환되는 배열의 크기는 5이며, 첫 번째 원소와
   * 마지막 원소가 동일한 지점을 가리킵니다.
   */
  ...
  public val locationTrackingMode: LocationTrackingMode?
    get() = when (map?.locationTrackingMode) {
      ...
      LocationTrackingMode.NoFollow -> LocationTrackingMode.NoFollow
      LocationTrackingMode.Follow -> LocationTrackingMode.Follow
      LocationTrackingMode.Face -> LocationTrackingMode.Face
      null -> null
    }
  ...
  // Guarded by `lock`.
  private var map: NaverMap? by mutableStateOf(null)
            ...
            val animateOnMapAvailable = object : OnMapChangedCallback {
              override fun onMapChangedLocked(newMap: NaverMap?) {
                if (newMap == null) {
                  ...
                  continuation.resumeWithException(CancellationException("internal error; no NaverMap available"))
