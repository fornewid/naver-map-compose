...
/**
 * 지도 상의 [MultipartPathOverlay]에 대한 [Composable]입니다.
 *
 * @param coordParts 좌표열 파트의 목록을 지정합니다. 만약 coordParts의 크기가 1 미만이거나 각 파트의 크기가 2 미만이면
 * [IllegalArgumentException]이 발생합니다. 각 파트에 null이거나 유효하지 않은([LatLng.isValid]가 false인) 좌표가
 * 있다면 [InvalidCoordinateException]이 발생합니다.
 * @param colorParts 색상 파트의 목록을 지정합니다. 만약 colorParts의 크기가 1 미만이거나 null인 파트가 있다면
 ...
 */
@Immutable
public data class ColorPart(
    public val color: Color = Color.White,
    public val outlineColor: Color = Color.Black,
    public val passedColor: Color = Color.White,
    public val passedOutlineColor: Color = Color.Black,
) {
    internal val value: MultipartPathOverlay.ColorPart
        get() = MultipartPathOverlay.ColorPart(
            color.toArgb(),
            outlineColor.toArgb(),
            passedColor.toArgb(),
