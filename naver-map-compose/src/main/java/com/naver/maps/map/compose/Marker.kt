...
/**
 * 지도 상의 [Marker]에 컨텐츠를 그리는 [Composable]입니다.
 *
 * @param keys 이 마커의 상태를 나타내는 고유 키입니다. 키 중 하나를 변경하면 content composable의 렌더링이 트리거되어
 * 업데이트된 마커가 그려집니다.
 * @param state position 및 정보 창과 같은 마커 상태를 제어하거나 관찰하는 데 사용되는 [MarkerState]입니다.
 * @param icon 아이콘을 지정합니다.
 * @param iconTintColor 아이콘에 덧입힐 색상을 지정합니다. 덧입힐 색상을 지정하면 덧입힐 색상이 아이콘 이미지의 색상과 가산
 ...
 * 오버레이만이 클릭 이벤트를 받을 수 있습니다. 예를 들어 마커와 지상 오버레이가 겹쳐져 있고 지상 오버레이에만 클릭 이벤트
 * 리스너가 지정된 경우, 사용자가 마커를 클릭하더라도 지상 오버레이가 클릭 이벤트를 받습니다.
 * @param content 마커에 전달하는 컨텐츠에 사용되는 composable 람다 표현식입니다.
 */
@ExperimentalNaverMapApi
@Composable
@NaverMapComposable
public fun MarkerComposable(
    ...
    val icon = rememberComposeOverlayImage(*keys) { content() }
    MarkerImpl(
        state = state,
        width = width,
        height = height,
